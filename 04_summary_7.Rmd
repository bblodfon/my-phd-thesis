...
:::{.quote}
```{=html}
<blockquote style='border:none'>&ldquo;If you can't even clean up your own room, who are you to give advice to the world?&rdquo; </br><strong> Jordan B. Peterson </strong></blockquote>
```
:::
...
Making the results of the simulations verifiable (i.e. by ensuring that the algorithmic procedures and the model equations are correctly coded ^[Read more on verification [here](#gtrr)]), will increase their reproducibility, further supporting the aforementioned goals [@Sandve2013].
All in all, there can only be gains if the code is *clean* and properly taken care of [@Martin2009].

:::{.quote}
```{=html}
<blockquote style='border:none'>&ldquo;The standard library saves programmers from having to reinvent the wheel.&rdquo;
</br><strong> Bjarne Stroustrup</strong></blockquote>
```
:::
```{=html}
...
To increase interoperability of our software, we used the Java library BioLQM [@Naldi2018] to enable the export of the produced Gitsbe models to standardized formats in the logical modeling community, such as GINML (used in GINsim, a software tool enabling the definition, analysis and simulation of regulatory graphs based on the logical formalism [@Naldi2018b]), SBML-qual (a standard designed for the representation of multivalued qualitative models of biological networks [@Chaouiya2013]) and BoolNet (file format of the models built with the BoolNet `R` package, used for the simulation and analysis of Boolean networks [@Mussel2010]).
...
One of these modules was `druglogics-synergy`, a Java package used to serially execute Gitsbe and Drabme in one go, and which was employed for the simulations of **[Paper 3](#Paper3)**.
...
The results of these efforts, along with biologically-relevant mechanistic insights derived from further analyzing the simulation data and performing various investigations, are analytically presented in **[Paper 3](#Paper3)**, and in even more detail at the `ags-paper` repository ^[See `ags-paper` repository [here](#misc-links)], which also includes reproducibility guidelines.
...
We continued by performing a thorough analysis using the CImbinator tool [@Flobak2017] and established a methodology which computed the synergy classification of the reference dataset that best matched our internal curation efforts to call synergy ^[See `sintef-obs-synergies` repository [here](#misc-links)].
...
A PKN that fits all the above characteristics was curated within our research group and refined throughout many years, resulting in the topology that was used for the simulations of **[Paper 3](#Paper3)** (CASCADE ^[See CASCADE repository [here](#misc-links)]).
In addition, proper signaling data was needed to train the Gitsbe models to a cancer proliferating phenotype.
For that purpose, we used the literature curated activity profile for a set of nodes in CASCADE, which was the result of a previous research effort [@Flobak2015].
...
```{r fig6, cache = TRUE, echo = FALSE, fig.cap='(ref:caption-fig6)', fig.pos='!ht'}
knitr::include_graphics(path = 'img/drabme_synergy_roc.png', auto_pdf = TRUE)
```

While exploring various configuration options for the Gitsbe and Drabme simulations ^[See DrugLogics software documentation [here](#doc-links)], we needed a tool to quickly assess their effect on the pipeline's performance and see which were the most important to tune.
...
This app is now part of the DrugLogics software suite ^[See `druglogics-roc` repository [here](#druglogics-soft-links)], facilitating the visualization of the pipeline's performance.
...
For example, we verified across many analyses ^[See `gitsbe-model-analysis` repository [here](#misc-links)] that the ERK node of the CASCADE signaling network was particularly overexpressed in the models that predicted most of the gold standard synergies in the AGS cell line from the reference drug combination dataset.
This was an interesting finding, since knowledge gathered from the scientific literature indicates conflicting measurements of ERK activity in AGS cells [@Flobak2015] and so our modeling results, upon further analysis, have the potential of providing useful information related to the studied biological system (this particular result is also shown using a different methodology in **[Paper 3](#Paper3)**).
...
This also motivated us to study how the diversity in particular Boolean rule assignments in the different model classes translates to the respective target nodes' activity (the link between node parameterization and activity was further investigated in **[Paper 5](#Paper5)**).
...
The result was the creation of the emba `R` package and its addition to the DrugLogics software suite (**[Paper 4](#Paper4)**).
