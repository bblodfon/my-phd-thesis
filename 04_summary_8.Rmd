...
:::{.quote}
```{=html}
<blockquote style='border:none'>&ldquo;That's all well and good in practice, but how does it work in theory?&rdquo; </br><strong> Shmuel Weinberger </strong></blockquote>
```
:::
...
Since parameterization is so important, how can we find which are the &ldquo;right&rdquo; rules or similarly, how can we distinguish between those that are right versus those that are not (or are less so), so that we can choose the former for our models?
...
On a more theoretical front, in Abou-Jaoudé et al. (2019) [@Abou-Jaoude2019], the authors formally describe the concept of logical bifurcation diagrams, a framework to assess how changes in the logical parameters result in the change of the dynamics of logical models.
...
We made a Java software package that can produce either a random sample or all possible Boolean models, based on a standardized equation form [@Mendoza2006] and its most basic variation (Figure \@ref(fig:fig7)).
The `abmlog` package is now also part of the DrugLogics software suite and was used in the analyses of **[Paper 5](#Paper5)** to show how the output behavior of the two alternative parameterization options of the Gitsbe models varies based on the number of regulators in the respective equations and the ratio of positive to negative regulators (as these were defined in the original PKN).
...
Exploiting the dimension reduction and visualization method UMAP [@McInnes2018], we constructed several Boolean model maps and were able to visualize model differences such as fitness to training data and prediction performance ^[See `bool-param-maps` repository [here](#misc-links)].
